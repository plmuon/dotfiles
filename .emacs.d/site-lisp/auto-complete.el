  ...
  (save-excursion
    (let ((overlay (ac-inline-overlay))
      ...
      (if (eq length 0)
          ;; Case: End-of-line
          (progn
            (put-text-property 0 1 'cursor 1))
           ...
           (unless (member prefix ignore-list)
             (save-excursion
               (setq point (cond
                            ...
                            ((stri                          (propertize (car candidate) 'value (cdr candidate))
                                   candidate))
                               candidates))
      (when do-cache
           ...
           (progn
             (if (and ac-use-comphist ac-comphist)
                 (if ac-show-menu
                   ...
                   (setq ac-common-part (if candidates (peate ac-point (popup-preferred-width ac-candidates) height)
...
`t', `remove-undo-boundary' has no effect."
  ...
  (when (not (equal string (buffer-substring ac-point (point))))
    ...
    ;; Sometimes, possible when omni-completion used, (insert)              (<= (popup-width ac-menu) (- preferred-width 10))
...
(defun ac-quick-help (&optional force)
  ...
  (when (and (or force
             ...
             (null ac-quick-help))
      (setq ac-quick-help
            (funcall (if (ac-quick-help-use-posdoc nil point nil 300))
          (popup-tip doc
                     :point point
                     :around t
                     :scroll-bar t
...
(defun ac-setup ()
  ...
  (unless ac-clear-variables-every-minute-timer
    (setq ac-clear-variables-every-min        (memq major-mode ac-modes))
      (auto-complete-mode 1)))
...
(defun ac-update-word-index ()
  ...
  r (buffer-local-value 'ac-word-index buffer))))
           into candidates
          ...
          (let ((file-name  (ac-symbol-file symbol 'defface)))
            ...
            (when file-name
              ...
              (princ (if (eq file-name 'C-source)
                       ...
                       (file-name-nondirectory file-name))))
            (princ "'.\n\n")
            (princ (or   '((candidates . ac-symbol-candidates)
